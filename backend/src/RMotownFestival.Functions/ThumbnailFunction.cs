...
namespace RMotownFestival.Functions
{
    public static class ThumbnailFunction
    {
        ...
        public static void Run([BlobTrigger("festivalpics/{name}", Connection = "BlobStorageConnection")]Stream image, 
            string name,
            ILogger log,
            [Blob("festivalthumbs/{name}", FileAccess.Write, Connection = "BlobStorageConnection")] Stream thumbnail)
        {
            log.LogInformation($"C# ThumbnailFunction trigger function Processed blob\n Name:{name} \n Size: {image.Length} Bytes");
